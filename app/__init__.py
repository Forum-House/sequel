...
def create_app():
    ...
    def get_users():
        with SessionLocal() as db:
            ...
            return jsonify([ud(u) for u in q.all()]),200
    @app.route("/users/bulk",methods=["POST"])
    def bulk_users():
        data=request.get_json(); filename=data.get("file","users.csv")
        with SessionLocal() as db:
            try:
    ...
    def get_urls():
        with SessionLocal() as db:
            ...
            return jsonify([urld(u) for u in q.all()]),200
    ...
    def get_events():
        with SessionLocal() as db:
            ...
            return jsonify([ed(e) for e in q.all()]),200
